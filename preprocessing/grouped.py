...
from preprocessing.load import df_grouped_train, df_grouped_test
...
if __name__ == "__main__":
    # Set up 
    nb_raw = df_grouped_test.shape[0]
    nb_col = df_grouped_test.shape[1]
    
    print(df_grouped_test.shape)
    
    df_grouped_cleaned_test = dataset_grouped_cleaner(df_grouped_test)
    
    print_values(df_grouped_cleaned_test)
    print_values_scope(df_grouped_cleaned_test)
    display_null(df_grouped_cleaned_test)