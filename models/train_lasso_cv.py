"""
Train Lasso Regression with Cross-Validation using ALL features.

This script uses nested cross-validation:
- Outer CV (5-fold): To evaluate model performance
- Inner CV: LassoCV automatically finds optimal alpha for each outer fold
"""
...
def main():
    ...
    print("Lasso Regression with Nested Cross-Validation")
    ...
    # Create pipeline with standardization and Lasso (with internal CV for alpha)
    print("\nCreating pipeline: StandardScaler + LassoCV")
    ...
    print("Note: LassoCV performs inner CV for alpha selection in each fold")
    ...
    print(f"  Features eliminated: {zero / len(coefficients) * 100:.2f}%")

    # Save model
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'saved_models')
    os.makedirs(save_dir, exist_ok=True)
    model_filename = os.path.join(save_dir, f"lasso_cv_all_features_{timestamp}.pkl")
    ...
    # Save CV results
    results_filename = os.path.join(save_dir, f"cv_results_lasso_{timestamp}.csv")
