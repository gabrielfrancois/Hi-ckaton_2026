"""
Train Linear Regression model with train/test split using ALL features.

This script trains a linear regression model using all available features
from the preprocessed dataset to compare performance against the top 9 features model.
"""
...
def main():
    ...
    print("Linear Regression Training Script with ALL Features")
    ...
    model, scaler, train_r2, test_r2, coef_df = train_and_evaluate(X_train, X_test, y_train, y_test)

    # Save model and scaler
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    save_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'saved_models')
    os.makedirs(save_dir, exist_ok=True)
    model_filename = os.path.join(save_dir, f"linear_regression_all_features_{timestamp}.pkl")
    scaler_filename = os.path.join(save_dir, f"scaler_all_features_{timestamp}.pkl")
    ...
    coef_filename = os.path.join(save_dir, f"coefficients_all_features_{timestamp}.csv")
