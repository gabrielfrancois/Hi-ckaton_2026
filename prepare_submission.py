...
def load_latest_test_data():
    """Load the most recent preprocessed test data."""
    ...
    # Extract IDs - they are in the first column (Unnamed: 0)
    if 'Unnamed: 0' in X_test.columns:
        test_ids = X_test['Unnamed: 0'].astype(int)
        X_test = X_test.drop('Unnamed: 0', axis=1)
    elif 'ID' in X_test.columns:
        test_ids = X_test['ID'].astype(int)
        X_test = X_test.drop('ID', axis=1)
    else:
        # Try to use the index as IDs
        test_ids = X_test.index
        print("Warning: Using index as IDs")
